...
export function renderAdvanced(
  ...
  if (element.kind === "smart-connector") {
    ...
    context.fillText(element.body[1] ?? "Target", bounds.x + bounds.width - 78, bounds.y + 48);
    context.restore();
    return;
  }
  ...
  if (element.kind === "section-zone") {
    ...
    context.fillRect(bounds.x + PAD, bounds.y + 42, bounds.width - PAD * 2, bounds.height - 54);
    context.setLineDash([8, 6]);
    context.strokeRect(bounds.x + PAD, bounds.y + 42, bounds.width - PAD * 2, bounds.height - 54);
    context.setLineDash([]);
  ...
  if (element.kind === "erd-relationship") {
    ...
    context.fillText(element.body[1] ?? "other.table_id", bounds.x + bounds.width * 0.52, bounds.y + 48);
    context.restore();
    return;
  }
  ...
  if (element.kind === "flow-step") {
    ...
    drawRoundedRectPath(context, bounds.x + PAD, bounds.y + 44, 54, bounds.height - 62, 12);
    context.fill();
    context.stroke();
  ...
  if (element.kind === "status-badge") {
    ...
    badges.forEach((badge, index) => {
      ...
      context.fillStyle = index === badges.length - 1 ? "rgb(34 197 94 / 22%)" : "rgb(148 163 184 / 18%)";
  ...
  if (element.kind === "annotation-pin") {
    ...
    context.fillStyle = "#dbeafe";
    context.font = "13px Inter, ui-sans-serif, system-ui, sans-serif";
    context.fillText(element.body[0] ?? element.title, centerX, bounds.y + bounds.height - 30);
    context.restore();
    return;
  }
  ...
  if (element.kind === "template-stamp") {
    ...
    context.strokeRect(bounds.x + PAD, bounds.y + 44, bounds.width - PAD * 2, bounds.height - 60);
    context.setLineDash([]);
  ...
  if (element.kind === "database-cylinder") {
    ...
    context.ellipse(centerX, bottomY, bounds.width * 0.32, 18, 0, Math.PI, 0, true);
    context.lineTo(centerX + bounds.width * 0.32, topY);
    context.stroke();
    context.textAlign = "center";
    context.fillText(element.title, centerX, topY + 34);
    textLines(context, element.body, centerX - bounds.width * 0.22, topY + 58, 17);
    context.restore();
    return;
  }
  ...
  if (element.kind === "org-card") {
    ...
    context.textAlign = "center";
    context.font = "800 16px Inter, ui-sans-serif, system-ui, sans-serif";
    context.fillText((element.title[0] ?? "T").toUpperCase(), bounds.x + 42, bounds.y + 62);
    context.fillStyle = "#dbeafe";
    context.textAlign = "left";
