...
export function PremiumStudioDialog({
  ...
  const buttonClass =
    "rounded-lg bg-control px-3 py-2 text-left text-sm text-text hover:bg-control/80";
      ...
      <div className="space-y-5 text-sm">
        <section>
          ...
          <div className="rounded-lg border border-border bg-surface-muted p-3 text-xs">
            ...
            <p>
              Locked:{" "}
              {scene.elements.filter((element) => element.locked).length}
            </p>
            <p>
              Groups:{" "}
              {
        ...
        <section>
          ...
          <div className="grid grid-cols-3 gap-2">
            <Button
              ...
              onClick={() => boardActions.insertTemplate("flowchart")}
            >
              Flowchart
            </Button>
            <Button
              className={buttonClass}
              onClick={() => boardActions.insertTemplate("mindmap")}
            >
              Mind map
            </Button>
            <Button
              className={buttonClass}
              onClick={() => boardActions.insertTemplate("roadmap")}
            >
              Roadmap
            </Button>
          </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button
              className={buttonClass}
              onClick={() =>
                setDiagnostics(boardActions.runDiagramDiagnostics())
              }
            >
              Run diagnostics
            </Button>
            <Button
              className={buttonClass}
              onClick={() => boardActions.applyBrandKit()}
            >
              Apply brand kit
            </Button>
            <Button
              className={buttonClass}
              onClick={() => boardActions.createSnapshot("Checkpoint")}
            >
              Create checkpoint
            </Button>
            <Button
              className={buttonClass}
              onClick={() => boardActions.restoreLatestSnapshot()}
            >
              Restore checkpoint
            </Button>
          </div>
